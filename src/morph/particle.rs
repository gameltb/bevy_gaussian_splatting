...
impl RenderAsset for GpuParticleBehaviorBuffers {
    ...
    fn prepare_asset(
        particle: Self::SourceAsset,
        render_device: &mut SystemParamItem<Self::Param>,
    ) -> Result<Self, PrepareAssetError<Self::SourceAsset>> {
        let particle_behavior_count = particle.0.len() as u32;
    ...
    fn asset_usage(particle: &Self::SourceAsset) -> RenderAssetUsages {
    ...
    fn run(
        ...
        for (
            ...
            for (
                ...
                let behaviors = world.get_resource::<RenderAssets<GpuParticleBehaviorBuffers>>().unwrap().get(behaviors_handle).unwrap();
